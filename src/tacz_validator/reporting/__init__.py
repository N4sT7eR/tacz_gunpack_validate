...
def write(report, path, fmt, locale="en"):
    """Write ``report`` to ``path`` in ``fmt``."""
    if fmt == "csv":
        return write_csv(report, path, locale)
    if fmt == "md":
        return write_markdown(report, path, locale)
    if fmt == "json":
        return write_json(report, path, locale)
    with open(path, "w", encoding="utf-8") as handle:
        handle.write(render_text(report, locale, colour=False))
    return path
